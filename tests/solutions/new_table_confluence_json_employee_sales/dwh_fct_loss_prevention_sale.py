...
def insert_stg_fct_loss_prevention_sale(**kwargs):
    """Insert and incremental load from source -> staging.stg_fct_loss_prevention_sale."""
    ...
    sql_save_increment_to = f"""
        SELECT
            adm.table_parameters_update('dwh.fct_loss_prevention_sale', 'increment_from_2', '{increment_to}');
    """

    sql_truncate = """
        TRUNCATE TABLE staging.stg_fct_loss_prevention_sale
    """

    sql_insert_staging = f"""
        INSERT INTO staging.stg_fct_loss_prevention_sale AS staging (
            date_key
            , store_key
            , item_key
            , time_key
...
def upsert_fct_loss_prevention_sale(**kwargs):
    """Upsert from stg -> dwh.fct_loss_prevention_sale."""
    ...
    sql_upsert_dwh = f"""
        ...
        SELECT
            ...
            , stg.time_key
            , stg.customer_key
            , stg.primary_store_key
            , stg.cashier_primary_store_key
            , stg.source_system
            , stg.sale_line_sid
            , stg.invoice_sid
            , stg.receipt_number
            ...
            , stg.cashier_id
            , stg.employee_id
            , stg.original_invoice_sid
            , stg.original_sale_line_sid
            , stg.billing_address_id
            , stg.shipping_address_id
            , stg.sale_channel
            , stg.is_employee_flag
            , stg.in_employment_period_flag
            , stg.return_in_return_period
            , stg.returned_greater_than_paid_flag
            , stg.employee_ea_flag
        FROM
            staging.stg_fct_loss_prevention_sale stg
        ON CONFLICT
...
with DAG(
    ...
    t_insert_stg_fct_loss_prevention_sale = PythonOperator(
        task_id='insert_stg_fct_loss_prevention_sale',
        python_callable=insert_stg_fct_loss_prevention_sale,
    )
...
t_insert_stg_fct_loss_prevention_sale >> t_upsert_fct_loss_prevention_sale
