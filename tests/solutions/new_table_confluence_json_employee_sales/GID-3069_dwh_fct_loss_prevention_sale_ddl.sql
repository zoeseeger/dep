/* ----------------------------------------------------------------- */
/*                    Create staging schema table                    */
/* ----------------------------------------------------------------- */
CREATE TABLE staging.stg_fct_loss_prevention_sale (
    date_key int4
    , store_key int4
    , item_key int4
    , time_key int4
    , customer_key int8
    , primary_store_key int4
    , cashier_primary_store_key int4
    , source_system text
    , sale_line_sid text
    , invoice_sid text
    , receipt_number text
    ...
    , cashier_id text
    , employee_id text
    , original_invoice_sid text
    , original_sale_line_sid text
    , billing_address_id text
    , shipping_address_id text
    , sale_channel text
    , is_employee_flag boolean
    , in_employment_period_flag boolean
    , return_in_return_period boolean
    , returned_greater_than_paid_flag boolean
    , employee_ea_flag boolean
);

-- Permissions --;
ALTER TABLE staging.stg_fct_loss_prevention_sale OWNER to role_master;
GRANT SELECT ON staging.stg_fct_loss_prevention_sale TO role_select_staging;
...
Subject Area: Loss prevention.
Updated: Monday 2PM.
2023-06-02 - zoeseeger - Created the table.';
...
-- Comments --;
COMMENT ON VIEW dwh.fct_loss_prevention_sale_v IS 'Description: xxx description.
Subject Area: Loss prevention.
Updated: Monday 2PM.
2023-06-02 - zoeseeger - Created the view.';
...
-- Permissions --;
ALTER VIEW dwh.fct_loss_prevention_sale_v OWNER TO role_master;
GRANT SELECT ON dwh.fct_loss_prevention_sale_v TO role_select_dwh;

-- Debugging lines --;
-- DROP VIEW IF EXISTS dwh.fct_loss_prevention_sale_v
...
-- DELETE FROM adm.table_parameters WHERE "table_name" = 'staging.stg_fct_loss_prevention_sale';
-- DROP TABLE IF EXISTS staging.stg_fct_loss_prevention_sale;
